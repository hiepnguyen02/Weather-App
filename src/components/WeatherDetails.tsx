...
function WeatherDetails({ currentCondition }) {
  ...
  const uv_index = (currentCondition?.uv_index??null);
  ...
  if (uv_index !== null) {
    if (uv_index >= 0 && uv_index <= 2) {
      ...
      uvIndexMessage2 = "Nguy hiểm, tìm chỗ có bóng râm, không tiếp xúc với ánh nắng.";
    }
  } else {
    uvIndexMessage = "Không có thông tin về chỉ số UV.";
  }

  const wind_kph = (currentCondition?.wind_kph??null);
  const wind_degree = (currentCondition?.wind_degree??null);
  let windMessage;
  const direction = ["Bắc", "Đông Bắc", "Đông", "Đông Nam", "Nam", "Tây Nam", "Tây", "Tây Bắc"];
  const index = Math.round(wind_degree / 45) % 8;
  windMessage = direction[index];

  const rainfall = (currentCondition?.rainfall);
  let rainfallMessage;
  ...
  const feelslike_c = (currentCondition?.feelslike_c);
  ...
  return (
    <View>
        <View style={styles.largeBox}>
          ...
          <View>
            <Text style={styles.detail}>Chỉ số không khí theo thang đo UK Defra.</Text>
          </View>
        </View>
